...
let Csoporttartalom = document.getElementById("Csoporttartalom");
...
let csoportgomb = document.getElementById("csoportgomb");
let csoportgombvissza = document.getElementById("csoportgombvissza");
let vissza2 = document.getElementById("visszagomb2");
document.getElementById("csoportnevtanar").style.display="none";
document.getElementById("tanarnevtanar").style.display="none";
document.getElementById("felhasznalokidtanar").style.display="none";
document.getElementById("kerdesekidtanar").style.display="none";
visszatanar.style.display = "none";
csoportgombvissza.style.display = "none";
...
var diakInfo2 = document.getElementById("diakInfo2");
var diakInfo3 = document.getElementById("diakInfo3");
let pontszam = 0;
let countdown;
...
diakInfo.style.display="none";
diakInfo2.style.display="none";
diakInfo3.style.display="none";
...
function Init() {
    ...
    if (sessionStorage.getItem("login")) {
        ...
        if (userType == "Admin") {
            ...
            diaknev=sessionStorage.getItem("fn");
        }
...
async function TTartalom() {
    ...
    let sqladat = await LekerdezesEredmenye("SELECT COUNT(*) as count FROM kerdesek INNER JOIN felhasznalok ON kerdesek.hozaado = felhasznalok.id WHERE felhasznalok.nev = '"+tanarnev+"'");
...
var csoporttorlesgomb=document.getElementById("csoporttorlesgomb");
csoporttorlesgomb.style.display="none";
async function Csoportok() {
    document.getElementById("csoportnevtanar").style.display="block";
    document.getElementById("tanarnevtanar").style.display="block";
    document.getElementById("felhasznalokidtanar").style.display="block";
    document.getElementById("kerdesekidtanar").style.display="block";

    var csoportid=document.getElementById("csoportid");
    csoportgomb.style.display = "none";
    csoportgombvissza.style.display = "block";
    let sqladat = await LekerdezesEredmenye("SELECT COUNT(*) as count FROM csoportok");
    
    for (let i = 1; i <= sqladat[0].count; i++) {
        let sql = "SELECT * FROM csoportok cs WHERE cs.id='" + i + "'";
        let valasz = await LekerdezesEredmenye(sql);
        
        if (valasz.length == 1) {
            let csoportnev = document.createElement("div");
            csoportnev.innerText = valasz[0].csoportnev;
            csoportnev.classList.add("valaszok");
            document.getElementById("csoportnev").appendChild(csoportnev);

            let tanarId = valasz[0].tanarnev;
            let sqlTanar = "SELECT f.nev FROM felhasznalok f WHERE f.id='" + tanarId + "'";
            let valaszTanar = await LekerdezesEredmenye(sqlTanar);
            
            if (valaszTanar.length == 1) {
                let tanarnev = document.createElement("div");
                tanarnev.innerText = valaszTanar[0].nev; 
                tanarnev.classList.add("valaszok");
                document.getElementById("tanarnev").appendChild(tanarnev);
            }

            let felhasznalokIds = valasz[0].felhasznalokid.split(",");
            felhasznalokIds.forEach(felhasznalokId => {
                let sqlName = "SELECT f.nev FROM felhasznalok f WHERE f.id='" + felhasznalokId.trim() + "'";
                LekerdezesEredmenye(sqlName).then(valaszName => {
                    if (valaszName.length == 1) {
                        let felhasznalokidDiv = document.createElement("div");
                        felhasznalokidDiv.innerText = valaszName[0].nev;
                        felhasznalokidDiv.classList.add("valaszok");
                        felhasznalokidDiv.addEventListener("click", function() {
                            csoportid.innerText =  felhasznalokId.trim();
                           
                        });
                        document.getElementById("felhasznalokid").appendChild(felhasznalokidDiv);
                    }
                });
            });

        
            let kerdesekIds = valasz[0].kerdesekid.split(",");
            kerdesekIds.forEach(kerdesekId => {
                let sqlQuestion = "SELECT k.kerdes FROM kerdesek k WHERE k.id='" + kerdesekId.trim() + "'";
                LekerdezesEredmenye(sqlQuestion).then(valaszQuestion => {
                    if (valaszQuestion.length == 1) {
                        let kerdesekidDiv = document.createElement("div");
                        kerdesekidDiv.innerText = valaszQuestion[0].kerdes; 
                        kerdesekidDiv.classList.add("valaszok");
                        kerdesekidDiv.addEventListener("click", function() {

                            csoportid.innerText =kerdesekId.trim();
                     
                        });
                        document.getElementById("kerdesekid").appendChild(kerdesekidDiv);
                    }
                });
            });
...
function CsoportokVissza(){
 
    csoportgomb.style.display = "block";
    csoportgombvissza.style.display = "none";
    document.getElementById("csoportnev").innerHTML = "";
    document.getElementById("tanarnev").innerHTML = "";
    document.getElementById("felhasznalokid").innerHTML = "";
    document.getElementById("kerdesekid").innerHTML = "";
    document.getElementById("csoportnevtanar").style.display="none";
    document.getElementById("tanarnevtanar").style.display="none";
    document.getElementById("felhasznalokidtanar").style.display="none";
    document.getElementById("kerdesekidtanar").style.display="none";
...
function Toplista(){
let sqltoplista = `SELECT t.nev, t.pont FROM toplista t JOIN felhasznalok f ON t.felhasznalokid = f.id WHERE f.admin != 1 AND f.tanar != 1 ORDER BY t.pont DESC LIMIT 10;`;
let sqltoplistadb = `SELECT COUNT(*) AS id FROM toplista t JOIN felhasznalok f ON t.felhasznalokid = f.id WHERE f.admin != 1 AND f.tanar != 1`;
    LekerdezesEredmenye(sqltoplistadb).then((valasz2)=> {
    LekerdezesEredmenye(sqltoplista).then((valasz)=> {
      
        if (diakInfo.innerHTML === "") {
            diakInfo.style.display="block";
            diakInfo2.style.display="block";
            diakInfo3.style.display="block";
            for (let i = 0; i < valasz2[0].id; i++) {
                diakInfo3.innerHTML += (i + 1) + ".<br>";
                diakInfo.innerHTML += valasz[i].nev +"<br>";
                diakInfo2.innerHTML += valasz[i].pont+ " pont <br>";
            }
        } 
        else {
            diakInfo.innerHTML = "";
...
function VisszaAMenube(){
    ...
    if(pontszam<5){
        ...
        sqlpont="UPDATE toplista SET pont = pont + 0 where nev='"+diaknev+"'";
        LekerdezesEredmenye(sqlpont);
    }
     
     
     else if(pontszam=5||5<pontszam<10){
        document.getElementById("nyertel").innerHTML = "A nyereményed 1000000 forint!";
        sqlpont="UPDATE toplista SET pont = pont + 1000000 where nev='"+diaknev+"'";
        LekerdezesEredmenye(sqlpont);
       
    }
     else if(pontszam=10||10<pontszam<15){
        document.getElementById("nyertel").innerHTML = "A nyereményed 10000000 forint!";
        sqlpont="UPDATE toplista SET pont = pont + 10000000 where nev='"+diaknev+"'";
        LekerdezesEredmenye(sqlpont);
    }
}
let currentRandomId; 
        ...
        if (valasztottId === valasz[0].helyesvalasz) {
            ...
            kellkerdes = true;
             ...
             if (pontszam === 15) {
                ...
                document.getElementById("ido").style.display="none";
                sqlpont="UPDATE toplista SET pont = pont + 40000000 where nev='"+diaknev+"'";
                LekerdezesEredmenye(sqlpont);
            }
        }
         else {
...
window.onload = function() {
    var elvezd = document.getElementById('elvezd');
    elvezd.volume = 0.1;
    elvezd.play();
}
